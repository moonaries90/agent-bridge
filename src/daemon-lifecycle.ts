...
//
// NOTE: this is resolved lazily inside launch() rather than at module top
// level. When bundled (bun build), top-level evaluation runs at import time,
// BEFORE kimi.ts / codex-kimi.ts have had a chance to set
// AGENTBRIDGE_DAEMON_ENTRY. That caused DAEMON_PATH to fall back to
// "./daemon.ts" resolved against the bundle location (dist/daemon.ts — which
// does not exist), and the detached daemon died silently. Resolving at call
// time reads the env the CLI set just before calling ensureRunning().
...
export class DaemonLifecycle {
  ...
  readStatus(): { proxyUrl?: string; controllerProxyUrl?: string; controlPort?: number; pid?: number } | null {
    try {
  ...
  private launch(): void {
    ...
    const daemonProc = spawn(process.execPath, ["run", daemonPath], {
      cwd: process.cwd(),
      env: {
        ...process.env,
