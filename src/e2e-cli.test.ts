...
describe("E2E: CLI surface", () => {
  ...
  test("agentbridge kill stops daemon, cleans state, and writes killed sentinel", async () => {
    await withHarness(async (harness) => {
      ...
      expect(result.stdout).toContain("Please restart (`abg claude`, `abg kimi`, `abg codex-kimi`, `abg zcode`, or `abg codex-zcode`)");
  ...
  test("bridge is killed and does not relaunch daemon after kill writes the killed sentinel", async () => {
    await withHarness(async (harness) => {
      ...
      // abg kill now sweeps ALL agentbridge processes (including the bridge
      // frontend), so the bridge is terminated, not left idle. The killed
      // sentinel still prevents any future relaunch.
      await sleep(1500);
      expect(bridge.child.exitCode).not.toBeNull();
      expect(harness.readLaunches()).toHaveLength(1);
      expect(existsSync(join(harness.stateDir, "killed"))).toBe(true);
    });
  }, 30000);
